...
export default async function TestnetHomePage() {
  ...
  const blocks = await getBlocks(latestBlockNumber, 6, network);
  const formattedBlocks = blocks.map(formatBlock);

  // Get recent transactions if Ponder is available (will return empty for testnet)
  let recentTxs: Array<{
    hash: string;
    from: string;
    to: string | null;
  ...
  if (ponderAvailable) {
    recentTxs = await getRecentTransactions(6, network);
  }
  ...
  return (
    <div className="container">
      ...
      <div style={{ marginBottom: "var(--space-xl)" }}>
        ...
        >
          <span style={{ color: "#F59E0B", animation: "cursorBlink 1s step-end infinite" }}>_</span>TESTNET EXPLORER
        </h1>
        <p style={{ color: "var(--text-muted)", fontSize: 14 }}>
          Real-time block explorer for Flow EVM Testnet (Chain ID: 545)
        </p>
      </div>
      <style>{`
      ...
      >
        {/* Recent Blocks */}
        <section>
          <SectionHeader title="Latest Blocks" href="/testnet/blocks" />
        ...
        {/* Recent Transactions */}
        <section>
          <SectionHeader title="Latest Transactions" href="/testnet/blocks" />
          <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
            {recentTxs.length > 0 ? (
              ...
              >
                Testnet transaction indexing not available
              </div>
            )}
          </div>
        </section>
      ...
      >
        ...
        />
        <span>This is Flow EVM Testnet. Tokens and transactions have no real value.</span>
      </div>
...
function BlockRow({
  ...
  return (
    <Link
      href={`/testnet/block/${block.number}`}
      style={{
        display: "flex",
        alignItems: "center",
        justifyContent: "space-between",
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: "var(--space-md)" }}>
        ...
        <div>
          <div className="mono" style={{ fontSize: 15, color: "#F59E0B", fontWeight: 500 }}>
